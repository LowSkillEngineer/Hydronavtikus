...
// ROM commands
static DS18B20_Command readRom = { .code = 0x33, .rxBytesNum = 8, .txBytesNum =
		0 };
static DS18B20_Command skipRom = { .code = 0xCC, .rxBytesNum = 0, .txBytesNum =
		0 };

// Function commands
static DS18B20_Command readScratchpad = { .code = 0xBE, .rxBytesNum = 9,
		.txBytesNum = 0 };
static DS18B20_Command writeScratchpad = { .code = 0x4E, .rxBytesNum = 0,
		.txBytesNum = 3 };
static DS18B20_Command convertT = { .code = 0x44, .rxBytesNum = 0, .txBytesNum =
		0 };
...
/*----------------------------------------------------------------------------*/
static DS18B20_Status ExecuteCommand(DS18B20 *sensor, DS18B20_Command command,
		uint8_t *data) {
...
/*----------------------------------------------------------------------------*/
DS18B20_Status DS18B20_ConvertT(DS18B20 *sensor,
		DS18B20_WaitCondition waitCondition) {
...
DS18B20_Status DS18B20_ReadScratchpad(DS18B20 *sensor) {
	...
	sensor->temperatureLimitHigh =
			rxData[DS18B20_SCRATCHPAD_T_LIMIT_H_BYTE_IDX];
	...
	uint16_t tRegValue = (rxData[DS18B20_SCRATCHPAD_T_MSB_BYTE_IDX] << 8)
			| rxData[DS18B20_SCRATCHPAD_T_LSB_BYTE_IDX];
...
DS18B20_Status DS18B20_ReadRom(DS18B20 *sensor) {
	...
	for (uint8_t i = 0; i < DS18B20_SERIAL_NUMBER_LEN_BYTES; i++) {
		sensor->serialNumber[i] =
				rxData[DS18B20_SERIAL_NUMBER_OFFSET_BYTES + i];
	}
...
float adc_to_pH(pH_sensor *sensor, ADC_HandleTypeDef *hadc) {
	float adc_resolution = 4095.0;
	float samples = 1.52867;
	sensor->adc = HAL_ADC_GetValue(hadc);
	HAL_Delay(100);
	sensor->voltage = 5 / adc_resolution * sensor->adc / samples;

	sensor->pH = 7 + ((2.5 - sensor->voltage) / 0.18);
	HAL_ADC_Stop(hadc);
	return sensor->pH;
}
/*----------------------------------------------------------------------------*/

float adc_to_turbidity(turbidity_semsor *sensor, ADC_HandleTypeDef *hadc,
		uint32_t InjectedRank) {
	float adc_resolution = 4095.0;
	float a = -290;
	float c = 3000;
	float samples = 1.52867;
	sensor->adc = HAL_ADCEx_InjectedGetValue(hadc, InjectedRank);
	HAL_Delay(100);
	sensor->voltage = 5 / adc_resolution * sensor->adc / samples;
	sensor->turbidity = a * sensor->voltage * sensor->voltage	+ 2 * (sensor->voltage + c);
	HAL_ADCEx_InjectedStop(hadc);
	return sensor->turbidity;
}

/*----------------------------------------------------------------------------*/
void adc_on(ADC_HandleTypeDef *hadc) {
	HAL_ADC_Start(hadc);
	HAL_ADCEx_InjectedStart(hadc);
	HAL_ADC_PollForConversion(hadc, 100);
	HAL_ADCEx_InjectedPollForConversion(hadc, 100);
}
