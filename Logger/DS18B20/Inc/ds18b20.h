...
typedef struct pH_sensor{
	uint16_t adc;
	float voltage;
	float pH;
}pH_sensor;

typedef struct turbidity_semsor{
	uint16_t adc;
		float voltage;
		float turbidity;
}turbidity_semsor;
...
extern float adc_to_pH(pH_sensor *sensor, ADC_HandleTypeDef* hadc);
extern float adc_to_turbidity(turbidity_semsor *sensor, ADC_HandleTypeDef* hadc, uint32_t InjectedRank);
extern void adc_on(ADC_HandleTypeDef* hadc);
